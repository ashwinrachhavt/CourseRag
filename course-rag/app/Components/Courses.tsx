...
export default function Course(randomCourse: Course) {
    ...
    return (
        <section className={`flex flex-col justify-center items-center transition-opacity ease-in-out duration-1000 ${fade ? "opacity-0" : "opacity-100"}`}>
            <div className="flex flex-col justify-center items-center">
                ...
                <p className={`text-2xl text-center mt-4 ${poppins.className}`}>Author: {randomCourse.author}</p>

                <p className={`text-2xl text-center mt-4 ${poppins.className}`}>Category: {randomCourse.category}</p>
            </div>

            <div className="grow mt-24">
                <p
                    className={`text-5xl text-center max-w-3xl leading-normal ${roboto.className}`}
                >{randomCourse.course}</p>
            </div>
        </section>
    )
}